...
public class Runner {
    ...
    protected void run(String location, boolean flag) {
        ...
        /*makeDir(resultFiles);
        final File heFile = makeFile(resultFiles, "HE.txt");
        final File ceFile = makeFile(resultFiles, "CE.txt");
        final File hcFile = makeFile(resultFiles, "HC.txt");*/
        //Using Divider class;
        //***************************************
        ...
        divider.runner(hFile, eFile, cFile);
...
*/
    ...
    private void finalResult(File baseFile, File destFile, File resultFile) {
    ...
    private void compareProbeToDestFile(ArrayList<String[]> baseMemory, File destFile, ArrayList<String[]> finalData, ArrayList<String[]> finalData2, long destLineNumbers) {
        ...
        try {
            ...
            while (counter <= destLineNumbers) {
                if (counter % 10000 == 0)
                    LOGGER.log(Level.INFO, (counter) + " : Gone in dest file - > " + destFile.getName());
                ArrayList<String[]> destMemory = makeProbe(destFile, counter, baseMemory.get(baseSize - 1).length);
                    ...
                    while (baseFilecounter < baseSize) {
                        ...
                        while ((destFileCounter < destSize) && (desStringSize == baseStringSize)) {
                            if (baseString.equals(destString) && !(basePdbid.equals(destPdbid))) {
                                LOGGER.log(Level.INFO, "Found sequence :" + destString);
                                DsspGui.txtAreaLog.append("Found sequence :" + destString + "\n");
                                DsspGui.txtAreaLog.update(DsspGui.txtAreaLog.getGraphics());
    ...
    private ArrayList<String[]> makeProbe(File baseFile, int counter, int cutSize) {
        ...
        try {
            ...
            if (currentLine != null) {
                ...
                String pdbId = temp[1].trim();
                start = Integer.parseInt(temp[2].trim());


                if (cutSize > sequence.length()) {
                    cutSize = sequence.length();
                }
                while (lentgh <= sequence.length()) {
                    //   while (endIndex <= sequence.length()) {

                    while (endIndex <= cutSize) {
    ...
    protected void writeData(File file, boolean keepFile, String data) {
        FileWriter writer;
        try {
            if (keepFile) {
